...
int main(int argc, char** argv)
{
	...
	std::string inputFile, outMeshFile = "a.obj", cageFile, cageDeformedFile, embeddedMeshFile, parameterFile, variant_string = "bbw", modelInfluenceFile, fbxFile;
	...
	desc.add_options()
		...
		("BGC", "Use Green coordinates of Bezier patches.")
	...
	const bool BGC = !lbc && !QGC && !harmonic && !green && static_cast<bool>(vm.count("BGC"));
	const bool QMVC = !BGC && !QGC && !lbc && !harmonic && !green && static_cast<bool>(vm.count("QMVC"));
	const bool MLC = !BGC && !QMVC && !QGC && !lbc && !harmonic && !green && static_cast<bool>(vm.count("MLC"));
	const bool MEC = !BGC && !MLC && !QMVC && !QGC && !lbc && !harmonic && !green && static_cast<bool>(vm.count("MEC"));
	...
	if (!MVC && !somigliana && !green && !BGC && !QMVC &&  !QGC && !MLC && !MEC && !vm.count("embedded"))
	{
		std::cerr << "You must specify an embedding!\n";
		return 1;
	}

	if (!somigliana && !MVC && !green && !QMVC && !QGC && !BGC && !MLC && !MEC)
	{
		if (verbosity)
		{
	...
	if (!interpolate_weights && find_offset && !MVC && !green && !QGC && !BGC && !MLC && !MEC && !somigliana)
	{
	...
	else if (!green && !QMVC && !QGC && !BGC && !MLC && !MEC && !somigliana && !MVC)
	{
	...
	if (!green && !MVC && !QMVC && !QGC && !BGC && !MLC && !MEC && verbosity)
	{
	...
	if (!lbc && !green && !QMVC && !QGC && !BGC && !MLC && !MEC)
	{
	...
	if (influence)
	{
		...
		if (!MVC)
		{
			write_influence_color_map_OBJ(base_name + "_influence_" + variant_string + ".obj", V_model, T_model, interpolate_weights ? W_interpolated : W,
				control_vertices_idx, (green || QGC || BGC || MLC || MEC || interpolate_weights) ? 0 : model_vertices_offset, green || QGC || BGC || MLC || MEC);
		}
#ifdef WITH_SOMIGLIANA	
		else
		{
